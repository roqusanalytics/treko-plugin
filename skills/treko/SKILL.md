...
- A Cloudflare bot wall (`navigate` returns `botChallenge` with `autoResolved:false`, or `recon` reports a `cloudflare` captcha) — follow the **pixel path** (see the Cloudflare pattern below).
...
Omit `tab` and every call lands on **your session's own tab** (see Parallel agents above) — that is the default and almost always what you want. Pass an explicit `tab` only to target a different one; it resolves in this order:
- Numeric index: `"0"`, `"1"`, …
...
| Full-page "Just a moment…" / "Performing security verification" interstitial | Cloudflare managed interstitial — the checkbox is beyond selectors' reach | Follow the **pixel path** (see the Cloudflare pattern below) |
...
2. `autoResolved:false` → take the **pixel path**: `screenshot` → read the "Verify you are human"
   checkbox's **center pixel coords** off the image → `captcha { action:"turnstile", x, y }`
   (at dpr=1 image px = viewport px) → `read` the title to confirm it changed off
   "Just a moment…" / "Tikriname jūsų naršyklę". This is the canonical pixel path — every other
   mention in this skill means exactly this sequence.
3. If it's an image / "select all squares" challenge, **surface to the user** — don't guess.

Why the pixel path exists: on full-page interstitials the visible checkbox sits in a closed
shadow DOM that selectors can't reach, so only screenshot+coords is reliable. For **embedded**
Turnstile widgets, plain `captcha { action:"turnstile" }` (no coords) auto-detects and clicks.
treko drives a real, non-headless Chrome, so a real mouse click passes it (synthetic clicks
don't), and a passed wall sets a clearance cookie so the site loads directly next time.
